...
class CifarClient(fl.client.NumPyClient):
    ...
    def fit(
        ...
        if config['start'] == True:
            ...
            if attribute == "isMaleisFemale":
                cifar.train_sud_gender(self.target_model, self.trainloader, epochs=1, device=DEVICE,lr_=0.01)
            elif attribute == "isRace":
                cifar.train_sud_race(self.target_model, self.trainloader, epochs=1, device=DEVICE,lr_=0.01)
            else:
...
def main() -> None:
    ...
    # Load model
    model = cifar.B_AlexNet().to(DEVICE).train()
    target_model = cifar.B_AlexNet().to(DEVICE)
